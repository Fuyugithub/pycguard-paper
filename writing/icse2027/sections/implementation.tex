...
evaluates retained obligations in deterministic order, stopping at the
first high-confidence violation per function. Only high-confidence
violated judgments become reports.
...
For each retained pair that reaches the LLM, the checker constructs a prompt with four components:
...
not eliminate model error, which we address through response validation,
manual review, and prompt ablation.
