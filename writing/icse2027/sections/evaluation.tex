...
The RQ1 pipeline queries each function with the specific obligation that
the static binder assigned to it, producing one targeted prompt per
binding. The ablation harness follows the same binding step but evaluates
each of the 86 true-positive cases as an independent unit, without the
surrounding context of sibling obligations in the same function. This
difference in query scope accounts for the 13-case gap between the
end-to-end result (86/86) and the ablation baseline (73/86); both
configurations use identical models, prompts, and confidence filters.
Removing the CoT system prompt drops recall to 74.4\% (64/86). Removing
the DC list keeps the total at 84.9\% but covers a non-identical set of
bugs. The scope difference affects both ablations equally and therefore
