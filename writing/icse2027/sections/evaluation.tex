...
We attempt to run PyRefcon over all 489 C/C++ source files and retain
warnings from files accepted by its compilation
...
project/file/function cases are used in both configurations; the
unpruned configuration sends all bound SOs for those cases to the LLM,
whereas the pruned configuration filters them before checking.
...
ultimately answer SAFE. The replay-level reduction is smaller than the
...
the pipeline fixed. \emph{Target} is the number of cases recovered by
the full PyCGuard configuration in the end-to-end run; all 86 are
recovered, so Target equals the RQ1 true-positive count.
...
prompt ablation shows that removing the CoT step drops recall to
74.4\% (64/86) and removing the discharge-condition list drops recall
to 84.9\% (73/86), each relative to the full configuration.
