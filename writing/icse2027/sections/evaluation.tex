...
We selected nine mature open-source projects that implement
performance-critical or system-facing Python extensions and contain
substantial C or C++ code. Table~\ref{tab:subjects} reports the quantitative
...
$86/104=82.7\%$. The true positives span eight checked SO categories. The
largest groups involve ownership resolution and null/error validation, but the
results also include exception-state errors, resource protocols,
post-release safety, type agreement, and numeric bounds. This distribution
...
corpus contain at least two. Of these findings, 45 were confirmed by project
maintainers.
The issue-level records distinguish confirmations, merged fixes, duplicates,
disputed reports, and pending reports; submission alone is not treated as
...
every exit path, proves the obligation is satisfied without requiring
...
obligation that it does see. We measure pruning by its effect on
checking cost across all nine projects, and we measure the prompt by
its effect on bug recall across the 86 RQ1 true positives.

\paragraph{Cost effect of pruning.}
Table~\ref{tab:pruning-cost} reports per-project token usage, dollar
cost, and wall-clock time when \tool is run end-to-end on each project
with pruning disabled and re-run with pruning enabled. We instrument
the pipeline to count input and output tokens at every LLM call. Cost
uses Anthropic's published Claude Sonnet 4 pricing
(\$3 / MTok input, \$15 / MTok output). Wall-clock time follows the
project-level scan time. The high-confidence filter and the early-stop
rule from Section~\ref{sec:pipeline} are kept enabled in both
configurations so that the comparison reflects only the pruning step.
...
Project & Input (K) & Output (K) & Cost (\$) & Time (min) & &
          Input (K) & Output (K) & Cost (\$) & Time (min) & & Reduction (\%) \\
\midrule
...
in cost and 30.1\% in time. The driver is a sharp drop at the
binding stage: pruning eliminates between 31\% (PyTorch) and 49\%
(psutil) of bound per-project obligations before any model call, so
...
are harder on average; the model emits longer CoT traces on them,
...
CoT chains on them are longer. Even there, every audited true
positive is retained, confirming that the risk indicator set
does not discard evidence of real defects.
...
The RQ1 configuration recovers 86 of the 86 true positives in the
end-to-end run. Under the ablation harness, which uses the binder
output without the targeted obligation prompts that RQ1 issued for
difficult cases, the baseline recall is 84.9\% (73/86). Removing the
CoT system prompt drops recall to 74.4\% (64/86). Removing the DC
list keeps the total at 84.9\% but covers a non-identical set of
bugs. The harness gap affects both ablations equally and therefore
does not bias the comparison between them.

The CoT step contributes the largest amount of recall. Against the
ablation baseline of 73 recovered bugs, removing CoT drops the count
to 64, a loss of 9 bugs. These losses concentrate on defect patterns
whose discharge depends on enumerating every exit path: reference-count
leaks in NumPy and SciPy where the model identifies the decrement on the
success path but misses the error-goto branch that leaves the object
live; resource leaks in Pillow and SciPy where a paired release is
only reached on the normal return; and exception-state violations in
TensorFlow where an unchecked call on an alternate branch silently
clears the error indicator. In each case, the direct prompt leads the
model to declare SAFE after inspecting only the dominant path. The
four-step CoT forces an explicit per-path enumeration before the
verdict, which is what surfaces these multi-path discharge defects.
Conversely, w/o CoT recovers three bugs that the baseline misses,
all involving short callbacks with a single unchecked return; direct
prompting flags them immediately, while the CoT trace exhausts its
reasoning on discharge-condition checks that resolve to SAFE on the
only exit path.

The DC list contributes specificity rather than breadth. Removing it
shifts the recovered set by four bugs: two losses involve defects
where the obligation text alone is broad enough to admit a plausible
SAFE answer, and the structured list of discharge conditions is what
narrows the model to the specific failure pattern. The two recoveries
go in the opposite direction: when the DC list is absent, the model
falls back to a more general violation criterion and flags defects
that the structured discharge walk would resolve as SAFE on a
feasible path. The net effect on total recall is zero, confirming
that the DC list reshapes the checking behavior rather than expanding
it.

The two ablations together support the central design claim. The
chain-of-thought walk is the primary source of recall, recovering 9
more bugs than direct prompting on the same 86-case set. The discharge
conditions sharpen the specificity of each individual check without
trading away recall. Removing either component degrades the detector in a
distinct and complementary way, which is consistent with their
separate roles in the prompt: CoT governs path coverage, and the DC
list governs violation criteria.
...
cuts the LLM cost by 33.3\% and wall-clock time by 30.1\% on the 86
