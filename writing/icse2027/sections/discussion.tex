...
Static pattern-based pruning is an analysis-cost mechanism rather than part of
the safety semantics. This distinction matters because a suspicious pattern is
neither necessary nor sufficient for a bug. The fallback that restores all obligations
when every pattern misses favors retention over maximum reduction. RQ3 tests the
remaining tradeoff by reporting both saved calls and known defects removed
before LLM checking. If a pattern loses a true defect, the proper response is to
extend or weaken the screen, not to redefine the obligation.
...
functions can hide ownership transfer. These cases account for several false
positives. Preprocessing selected macros, summarizing local helpers, and
modeling common RAII wrappers could expose the missing discharge evidence
without broadening the property scope.
...
every finding. The final artifact should include report-level labels and
